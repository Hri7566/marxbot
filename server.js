...
var Kings = ranks.kings;
var Nobles = ranks.nobles;
var Knights = ranks.knights;
var blacklist = ranks.blacklist;

function saveRanks() {
    fs.writeFileSync(path.join(__dirname, "ranks.json"), JSON.stringify({kings: Kings, nobles: Nobles, knights: Knights, blacklist: blacklist}, null, 4));
}
...
function getPart(boop) {
    for (const id in client.ppl) {
        ...
        if (part.name.toLowerCase().indexOf(boop.toLowerCase()) !== -1 || part._id.toLowerCase().includes(boop.toLowerCase())) {
            return part;
            break;
        }
    }
...
function sendChat(str) {
    client.sendArray([{m: 'a', message: `\u034f${str}`}]); // invis char
}
    ...
    if  (cmd == "/chatbot") {
        if (!!isKing) {
            ...
            if (toggle.toLowerCase() == 'on') {
                if (enabled == true) {
                    sendChat("The chat bot is already on.");
                } else {
                    enabled = true
                    sendChat("Chat bot is now on.");
                }
            } else if (toggle.toLowerCase() == 'off') {
                if (enabled == false) {
                    sendChat("The chat bot is already off");
                } else {
                    enabled = false
                    sendChat("Chat bot is now off.");
                }
            }
        }
        ...
        if (!isBlocked) {
            ...
            if (cmd == "/help") {
                // var phelp = ("Commands: /help | /about | /rank | /quote | /fact | /8ball | /math | /art | /eat | /kill | /stab | /hug | /shoot | /slap | / cuddle | /friendzone | /fight | /tickle | /roast | /poke | /catyears | /dogyears | /rps");
                // var khelp = (" | /art");
                // var nhelp = (" | /clearchat | /boom | /kickban | /wave ");
                // var kinghelp = (" | /on | /off");
                // if (isKing) {
                //     sendChat(phelp + khelp + nhelp + kinghelp);
                // } else if (isNoble) {
                //     sendChat(phelp + khelp + nhelp);
                // } else if (isKnight) {
                //     sendChat(phelp + khelp);
                // } else {
                //     sendChat(phelp);
                // }

                sendChat("Commands: /help | /about | /rank | /quote | /fact | /8ball | /math | /art | /eat | /kill | /stab | /hug | /shoot | /slap | / cuddle | /friendzone | /fight | /tickle | /roast | /poke | /catyears | /dogyears | /rps");
                if (isNoble || isKing) sendChat("Nobles Commands: /kickban | /clearchat");
                if (isKing) sendChat("King Commands: /chatbot | /goto | /getcrown | /js | /stop");
            } else if (cmd == "/rank") {
                var rank = isKing ? "Kings" : isNoble ? "Nobles" : isKnight ? "Knights" : "Peasants";
                sendChat(`Ranks are classed into three types, Kings, Nobles, Knights, and Peasants. King being the highest with all commands, and peasants being the lowest with only basic commands. ${msg.p.name}, your rank is ${rank}.`);
            } else if (cmd == "/art") {
                if (isKing) {
                ...
                try {
                    ...
                    sendChat(`${x} years in dog years: ${x * 7}`);
                } catch (err) {
                    sendChat(`Usage: /dogyears x`);
                }
            } else if (cmd == "/catyears") {
                try {
                    var x = parseFloat(args[1]);
                    sendChat(`${x} years in cat years: ${x * 15}`);
                } catch (err) {
                    sendChat(`Usage: /catyears x`);
                }
            } else if (cmd == "/stab") {
                try {
                    if (!argcat || part._id == msg.p._id) {
                ...
                if (isNoble == true || isKing == true) {
                    t=0; Object.keys(MPP.piano.keys).forEach(function(key) {setTimeout(function () {press(key,1)}, t++ * 70)});
                    t=0; Object.keys(MPP.piano.keys).forEach(function(key) {setTimeout(function () {press(key,1)}, t++ * 65)});
                    t=0; Object.keys(MPP.piano.keys).forEach(function(key) {setTimeout(function () {press(key,1)}, t++ * 60)});
                    t=0; Object.keys(MPP.piano.keys).forEach(function(key) {setTimeout(function () {press(key,1)}, t++ * 55)});
                    t=0; Object.keys(MPP.piano.keys).forEach(function(key) {setTimeout(function () {press(key,1)}, t++ * 50)});
                    t=0; Object.keys(MPP.piano.keys).forEach(function(key) {setTimeout(function () {press(key,1)}, t++ * 70)});
                    t=0; Object.keys(MPP.piano.keys).forEach(function(key) {setTimeout(function () {press(key,1)}, t++ * 65)});
                    t=0; Object.keys(MPP.piano.keys).forEach(function(key) {setTimeout(function () {press(key,1)}, t++ * 60)});
                    t=0; Object.keys(MPP.piano.keys).forEach(function(key) {setTimeout(function () {press(key,1)}, t++ * 55)});
                ...
                if (msg.args[1]) {
                    ...
                    sendChat("You must provide a question.");
                }
            } else if (cmd == "/eat") {
                if (!args[1]) {
...
client.on('a', msg => {
    ...
    if (cmd == '/math') {
        ...
        try {
            ...
            sendChat('Usage: /div x y');
        }
    } else if (cmd == '/pow' || cmd == '/exp') {
        try {
            let x = parseFloat(args[1]);
            let y = parseFloat(args[2]);
            sendChat(`Answer: ${x ** y}`);
        } catch (err) {
            sendChat('Usage: /pow x y');
        }
    } else if (cmd == '/atan') {
        try {
            let x = parseFloat(args[1]);
            sendChat(`Answer: ${math.atan(x)}`);
        } catch (err) {
            sendChat('Usage: /atan x');
        }
    } else if (cmd == '/atanh') {
        try {
            let x = parseFloat(args[1]);
            sendChat(`Answer: ${math.atanh(x)}`);
        } catch (err) {
            sendChat('Usage: /atanh x');
        }
    } else if (cmd == '/atan2') {
        try {
            let x = parseFloat(args[1]);
            let y = parseFloat(args[2]);
            sendChat(`Answer: ${Math.atan2(x, y)}`);
        } catch (err) {
            sendChat('Usage: /atan2 x y');
        }
    } else if (cmd == '/acos') {
        try {
            let x = parseFloat(args[1]);
            sendChat(`Answer: ${Math.acos(x)}`);
        } catch (err) {
            sendChat('Usage: /acos x');
        }
    } else if (cmd == '/acosh') {
        try {
            let x = parseFloat(args[1]);
            sendChat(`Answer: ${Math.acosh(x)}`);
        } catch (err) {
            sendChat('Usage: /acosh x');
        }
    } else if (cmd == '/cos') {
        try {
            let x = parseFloat(args[1]);
            sendChat(`Answer: ${Math.cos(x)}`);
        } catch (err) {
            sendChat('Usage: /cos x');
        }
    } else if (cmd == '/cosh') {
        try {
            let x = parseFloat(args[1]);
            sendChat(`Answer: ${Math.cosh(x)}`);
        } catch (err) {
            sendChat('Usage: /cosh x');
        }
    } else if (cmd == '/asin') {
        try {
            let x = parseFloat(args[1]);
            sendChat(`Answer: ${Math.asin(x)}`);
        } catch (err) {
            sendChat('Usage: /asin x');
        }
    } else if (cmd == '/asinh') {
        try {
            let x = parseFloat(args[1]);
            sendChat(`Answer: ${Math.asinh(x)}`);
        } catch (err) {
            sendChat('Usage: /asinh x');
        }
    } else if (cmd == '/cbrt') {
        try {
            let x = parseFloat(args[1]);
            sendChat(`Answer: ${Math.cbrt(x)}`);
        } catch (err) {
            sendChat('Usage: /cbrt x');
        }
    } else if (cmd == '/sqrt') {
        try {
            let x = parseFloat(args[1]);
            sendChat(`Answer: ${Math.sqrt(x)}`);
        } catch (err) {
            sendChat('Usage: /sqrt x');
        }
    } else if (cmd == '/tan') {
        try {
            let x = parseFloat(args[1]);
            sendChat(`Answer: ${Math.tan(x)}`);
        } catch (err) {
            sendChat('Usage: /tan x');
        }
    } else if (cmd == '/tanh') {
        try {
            let x = parseFloat(args[1]);
            sendChat(`Answer: ${Math.tanh(x)}`);
        } catch (err) {
            sendChat('Usage: /tanh x');
        }
    } else if (cmd == '/sin') {
        try {
            let x = parseFloat(args[1]);
            sendChat(`Answer: ${Math.sin(x)}`);
        } catch (err) {
            sendChat('Usage: /sin x');
        }
    } else if (cmd == '/sinh') {
        try {
            let x = parseFloat(args[1]);
            sendChat(`Answer: ${Math.sinh(x)}`);
        } catch (err) {
            sendChat('Usage: /sinh x');
        }
    } else if (cmd == '/string') {
        try {
            sendChat(`Answer: ${math.evaluate(argcat)}`);
        } catch (err) {
            sendChat('Usage: /string eval');
        }
    } else if (cmd == '/pi') {
        sendChat(Math.PI);
    } else if (cmd == '/e') {
